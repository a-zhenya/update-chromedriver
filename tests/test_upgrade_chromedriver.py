...
def run_upgrade(
    args=None, env=None,
    chrome=NEW_VERSION, chromedriver=OLD_VERSION, tools=True,
    update_found=True, curl_success=True, apt_version=NEW_VERSION, unzip_success=True,
    wrkdir=".", 
):
...
def test_driver_already_up_to_date(tmp_path):  # no need to upgrade
    # spoil curl here
    res, bindir, homedir = run_upgrade(chromedriver=NEW_VERSION, wrkdir=tmp_path)
...
def test_target_dir_change(tmp_path):  # custom target
    ...
    tdir.mkdir()
    res, bindir, homedir = run_upgrade(args=["--target-dir", str(tdir)], chromedriver=False, wrkdir=tmp_path)
    assert (tdir / "chromedriver").exists()
    assert res.returncode == 0

def test_download_dir_change(tmp_path):  # custom download dir
    ddir = tmp_path / "downloads"
    ddir.mkdir()
    res, bindir, homedir = run_upgrade(args=["--download-dir", str(ddir)], chromedriver=False, wrkdir=tmp_path)
    assert res.returncode == 0

def test_platform_specified(tmp_path):  # platform specified
    res, bindir, homedir = run_upgrade(args=["--platform", "linux64"], chromedriver=False, wrkdir=tmp_path)
    assert res.returncode == 0

def test_cli_version(tmp_path):  # -v version
    res, bindir, homedir = run_upgrade(args=["-v", NEW_VERSION], chromedriver=False, wrkdir=tmp_path)
    assert "Found downloadable chromedriver" in res.stdout

def test_version_without_flag(tmp_path):  # bare version
    res, bindir, homedir = run_upgrade(args=[NEW_VERSION], chromedriver=False, wrkdir=tmp_path)
    assert "Found downloadable chromedriver" in res.stdout

def test_version_from_apt(tmp_path):
    res, bindir, homedir = run_upgrade(args=["--apt"], chromedriver=False, wrkdir=tmp_path)
...
def test_download_only(tmp_path):
    res, bindir, homedir = run_upgrade(args=["--download-only"], chromedriver=False, wrkdir=tmp_path)
    assert res.returncode == 0

def test_leave_zip_on_failure(tmp_path):
    res, bindir, homedir = run_upgrade(args=["--leave-zip-on-failure"], unzip_success=False, chromedriver=False, wrkdir=tmp_path)
    assert "Leaving ZIP archive" in res.stdout
    assert res.returncode != 0
